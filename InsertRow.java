...
public class InsertRow {
    ...
    public void marshall() throws IOException {
        /* 
         * Write offsets of values, and the values themselves
         * to the buffer.
         */
        int currentOffset = this.offsets.length * 2;
        ...
        for (int i = 0; i < this.table.numColumns(); i++) {
            ...
            boolean primKey = col.isPrimaryKey();
            ...
            if (primKey) {
                ...
                // Advance currentOffset by appropriate amount
                if (colType != Column.VARCHAR) {
                    currentOffset += col.getLength();
                }
                else {
                    int lenVarChar = ((String)this.columnVals[i]).length();
                    currentOffset += lenVarChar;
                }
            }
            // Offset for end of record
            if (i == this.table.numColumns() - 1) {
                offsets[i + 1] = currentOffset;
            }
        }
        
        // Write offsets to value buffer
        for (int i = 0; i < this.offsets.length; i++) {
            this.valueBuffer.writeShort(this.offsets[i]);
        }
        
        // Write values to key and value buffers
        for (int i = 0; i < this.table.numColumns(); i++) {
            Column col = this.table.getColumn(i);
            ...
            // Primary key values should be written to key buffer
            if (primKey) {
                ...
                else if (colType == Column.INTEGER) {
