...
public class TableIterator {
    ...
    public boolean next() throws DeadlockException, DatabaseException {
        ...
        while (true) {
            ...
            // Stop when we no longer successfully move to next tuple
            if (ret != OperationStatus.SUCCESS) {
                return false;
            }
            
            // Stop looping if the where clause is satisfied
            if (this.where.isTrue()) {
                this.numTuples++;
                return true;
            }
    ...
    public Object getColumnVal(int colIndex) {
        /* 
         * Logic for getting the value at the column in the specified row
         */
        ...
        int[] offsets = new int[table.numColumns() + 1];
        
        for (int i = 0; i < offsets.length; i++) {
            offsets[i] = valueIn.readNextShort();
        }
        
        int startOffset = offsets[colIndex];
        // Case for if offset of column is primary key
        if (startOffset == -1) {
            RowInput keyIn = new RowInput(this.key.getData());
            
            if (colType == Column.INTEGER) {
                int key = keyIn.readNextInt();
                return key;
            }
            else if (colType == Column.REAL) {
                double key = keyIn.readNextDouble();
                return key;
            }
            else {
                int keyLen = this.key.getSize();
                String key = keyIn.readNextBytes(keyLen);
                return key;
            }
        }
        // Case for if offset of column is null
        if (startOffset == -2) {
            return null;
        }
        
        if (colType == Column.INTEGER) {
            int value = valueIn.readIntAtOffset(startOffset);
            return value;
        }
        else if (colType == Column.REAL) {
            double value = valueIn.readDoubleAtOffset(startOffset);
            return value;
        }
        else {
            int length = offsets[colIndex + 1] - startOffset;
            String value = valueIn.readBytesAtOffset(startOffset, length);
            return value;
        }
    }
