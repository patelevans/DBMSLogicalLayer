...
public class SelectStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
        ...
        try {
             ...
             */
            if (this.numTables() > 1) {
                throw new Exception("more than one table specified");
            }
            if (this.numColumns() > 0) {
                throw new Exception("cannot select specific columns");
            }
            
            Table table = this.getTable(0);
            // Attempt to open the table
            OperationStatus ret = table.open();
            if (ret != OperationStatus.SUCCESS) {
                throw new Exception("table failed to open");
            }
